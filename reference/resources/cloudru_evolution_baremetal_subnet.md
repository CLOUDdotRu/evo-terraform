...
resource "cloudru_evolution_baremetal_subnet" "resource_subnet" {
  ...
  # Поле address является неизменяемым. При изменении значения ресурс будет пересоздан.
  ...
  # Поле is_primary является неизменяемым. При изменении значения ресурс будет пересоздан.
  is_primary = false
  vlan_id    = 1
