...
resource "cloudru_evolution_mk8s_node_pool" "resource_node_pool" {
  ...
  remote_access = {
    # Поле ssh_key_id является неизменяемым. При изменении значения ресурс будет пересоздан.
    ssh_key_id = "00000000-0000-0000-0000-000000000000"
    # Поле username является неизменяемым. При изменении значения ресурс будет пересоздан.
    username = "cloudru"
  }
  auto_repair = {
    enabled = true
